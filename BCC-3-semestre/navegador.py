    ...
    for i in soup.find_all('a', attrs={"class": "category-page__member-link"}):
        ...
        if infos != None:
            ...
            if tipo != 'None':
                   ...
                   gravar(nome,tipo_arm,tipo_atq,d_fis.replace("-", "0"),d_mag.replace("-", "0"),d_fog.replace("-", "0"),d_ele.replace("-", "0"),d_crt,r_fis,r_mag,r_fog,r_ele,stabil,durab,wei,b_str,b_dex,b_int,b_fht,r_str.replace("-", "0"),r_dex.replace("-", "0"),r_int.replace("-", "0"),r_fht.replace("-", "0"))     
